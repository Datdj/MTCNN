...
from PNet.model import pnet_predict_v2
...
def main():
    ...
    for i in tqdm(range(num_faces)):
        ...
        # Create R-Net input
        rnet_input = list()
        for _bbox in c_bboxes:
            # print(bbox)
            # exit()
            rnet_input.append(crop_and_resize_v2(img, _bbox, 24))
    ...
    # exit()
        ...
        for window in windows:
            ...
            # Crop out the landmark faces and resize them to 48 x 48            
            lm_images.append(crop_and_resize_v2(img, window, 48))
        ...
        if (i + 1) % 20000 == 0:
            ...
            # Save the images
            np.save(args.output_directory + '/lm_images_' + str(batch) + '.npy', np.asarray(lm_images))
            lm_images = list()

            # Save the normalized landmarks into a numpy file
            np.save(args.output_directory + '/landmarks_' + str(batch) + '.npy', np.asarray(norm_landmarks))
            norm_landmarks = list()
