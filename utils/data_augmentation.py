...
def augment(images, labels):
    ...
    # Convert images from integer [0, 255] to float [0, 1)
    images = tf.cast(images, tf.float64) / 255
    ...
    random_indices = tf.where(tf.random.uniform([batch_size], 0, 2, tf.int32))
    images = tf.tensor_scatter_nd_update(images, random_indices, tf.clip_by_value(tf.image.random_contrast(tf.gather_nd(images, random_indices), 0.95, 1.05), 0, 1 - 1e-8))

    # Randomly adjust the brightness
    random_indices = tf.where(tf.random.uniform([batch_size], 0, 2, tf.int32))
    images = tf.tensor_scatter_nd_update(images, random_indices, tf.clip_by_value(tf.image.random_brightness(tf.gather_nd(images, random_indices), 0.05), 0, 1 - 1e-8))
