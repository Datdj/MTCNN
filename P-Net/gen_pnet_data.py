...
def main():
    ...
    pos_class = np.concatenate((np.ones((num_pos, 2)), np.zeros(num_pos, 1)), axis=1)
    par_class = np.zeros((num_par, 3))
    neg_class = np.concatenate((np.ones((num_neg, 1)), np.zeros((num_neg, 1)), np.ones((num_neg, 1))), axis=1)
    lm_class = np.zeros((num_lm, 3))
    ...
    # Shuffle everything
    rng = default_rng()
    random_indices = rng.permutation(images)
